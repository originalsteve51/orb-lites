...
try:
    ...
    while True:
        ...
        if target_region_num > len(spin_regions) or target_region_num < 0:
            top_to_bottom = not top_to_bottom
            cycle_color = random.choice([RED,WHITE,BLUE])

        for region_num, spin_region in enumerate(spin_regions):
            led_strand.set_region(spin_region)
            if region_num == target_region_num:
                led_color = cycle_color
            else:
                led_color = DARK    
            for led_num in range(0,8):
                an_led = spin_region.get_led_num(led_num)
                an_led.set_color(led_color)
