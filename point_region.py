...
#
# This class, PointRegion, maps individual LEDs rather than a range of LEDs.
import random
from led import LED

brightness = 200

class PointRegion(object):
	def __init__(self, region_name, strand_range, rgb):
		...
		for n in strand_range:
			self.map[idx] = n
			self.leds.append(LED(idx, n, rgb[0], rgb[1], rgb[2], brightness))
			idx = idx + 1
	...
	def set_led_num(self, idx, rgb, rgb_bright=100):
		led = self.leds[idx]
		led.r = rgb[0]
		led.g = rgb[1]
		led.b = rgb[2]		
	...
	def rotate(self, direction='ccw'):
		if direction == 'ccw': 
			overflow = self.leds[0]
			self.leds = self.leds[1:] 
			self.leds.append(overflow)
			self.renumber(self)
		else:        		    
			overflow = self.leds[len(self.leds)-1]
			self.leds = self.leds[:len(self.leds)-1] 
			self.leds.insert(0,overflow)
			self.renumber(self)        		    
            		
	def rotate_fill(self, direction='ccw'):
		if direction == 'ccw': 
			overflow = self.leds[0]
			self.leds = self.leds[1:] 
			self.leds.append(overflow)
			self.leds[0] = overflow
			self.renumber(self)
		else:        		    
			overflow = self.leds[len(self.leds)-1]
			self.leds = self.leds[:len(self.leds)-1] 
			self.leds.insert(0,overflow)
			self.leds[len(self.leds)-1] = overflow
			self.renumber(self)        		    

	def set(self, led, rgb, rgb_bright=brightness):
